<section class="feature-sec">
    <div class="container">
        <div class="row">
            <div class="col-md-4">
                <a href="#">
                    <div class="feature-icon">
                        <div><span><i class="fa fa-check" aria-hidden="true"></i></span></div>
                        <h2>@lang('message.home.services.t1')</h2>
                        <p>@lang('message.home.services.s1')</p>
                    </div>
                </a>
            </div>

            <div class="col-md-4">
                <a href="#">
                    <div class="feature-icon">
                        <div>
                            <span><i class="fa fa-cart-arrow-down" aria-hidden="true"></i></span></div>
                        <h2>@lang('message.home.services.t2')</h2>      <p>@lang('message.home.services.s2')</p>
                    </div>
                </a>
            </div>

            <div class="col-md-4">
                <a href="#">
                    <div class="feature-icon">
                        <div><span><i class="fab fa-modx" aria-hidden="true"></i></span></div>
                        <h2>@lang('message.home.services.t3')</h2>
                        <p>@lang('message.home.services.s3')</p>
                    </div>
                </a>
            </div>

            <div class="col-md-4">
                <a href="#">
                    <div class="feature-icon">
                        <div><span><i class="fas fa-plane" aria-hidden="true"></i></span></div>
                        <h2>@lang('message.home.services.t4')</h2>
                        <p>@lang('message.home.services.s4')</p>
                    </div>
                </a>
            </div>
            
            <div class="col-md-4">
                <a href="#">
                    <div class="feature-icon">
                        <div><span><i class="fas fa-shield-alt" aria-hidden="true"></i></span></div>

                            <h2>@lang('message.home.services.t6')</h2>
                        <p>@lang('message.home.services.s6')</p>
                    </div>
                </a>
            </div>
        </div>
    </div>
</section>